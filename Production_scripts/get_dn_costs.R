...
## Extract numbers of contacts from the CHAD - District Nursing Datamart using the query: DN-Contacts-Numbers-for-Costs
...
## save outfile ##
# .zsav
#haven::write_sav(dn_raw_costs,
#                 paste0(get_slf_dir(), "/Costs/raw_dn_costs.sav"),
#                 compress = TRUE
#)
...
dn_raw_costs_contacts <- readr::read_csv(paste0(get_slf_dir(), "/Costs/DN-Contacts-Numbers-for-Costs.csv"))
...
dn_raw_costs_contacts <-
  ...
  rename(HB2019 = "Treatment NHS Board Code 9",
...
# run if not currently loaded from above #
# read raw costs
# dn_raw_costs <- haven::read_sav(paste0(get_slf_dir(), "/Costs/raw_dn_costs.sav"))
...
## save outfile ##
# .zsav
#haven::write_sav(dn_raw_costs_contacts,
#                 paste0(get_slf_dir(), "/Costs/raw_dn_costs_with_contacts.sav"),
#                 compress = TRUE
#)
...
# run if not currently loaded from above #
# read raw costs with contacts
# dn_raw_costs_contacts <- haven::read_sav(paste0(get_slf_dir(), "/Costs/raw_dn_costs_with_contacts.sav"))
...
data <-
  data %>%
  mutate(uplift = 0) %>%
  mutate(tempyear1 = case_when(Board_Name == "NHS Highland" & year == "1617"                ~ "1819",
  ...
  mutate(uplift1 = case_when(Board_Name == "NHS Highland" & tempyear1 == "1819"                  ~ 2,
...
costs_lookup <-
  current_file %>%
  rename(cost_old = "cost_total_net",
         year = "Year")
