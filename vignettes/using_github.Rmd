---
title: "02 Using GitHub to Develop createslf"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{02 Using GitHub to Develop createslf}
...
The `master` branch is kept as the 'production' version of the code which is the code used to run the last quarterly update. This should only be merged into after an update has been released. For example, release of `march25-update` complete, then this should merge into `master` during the tidy up of github.

During a quarterly update, we will create a development branch related to the update. For example, a recommended naming convention is:`march25-update`. Any new changes should be first merged into the `XXX-update` branch, this way all changes made are kept separate until they have been properly tested in a full run of the files. 


### Create a new branch

To work on a new issue, you will need to create a new branch. This is best done for even small changes such as renaming scripts/functions. Typically, issues will relate to the update we are working on. Therefore, you should first pull the `XXX-update` branch to get the most up-to-date version and create a new branch off of this. 
...
Click 'create a branch' - by default, the 'branch source' will be `master`, so you will usually want to click 'change branch source' and select `XXX-update` (or otherwise).
...
Merges into `XXX-update` need to be done through a PR (and GitHub will stop you if you try to do otherwise!) In the usual case, there should be one person in charge/making changes in each branch. A PR should only be submitted to `master` once the quarterly update has been released. This ensures that `master` is the most up to date version and we are confident the code is working. 
...
There are a number of reasons you might have issues when trying to push to GitHub:
 ...
 - You started your branch from `master` but then opened a PR against `XXX-update` and clicked 'update branch'. Or some other combination of branch cross-contamination.
