...
Get the full path to the Care Home name lookup, which has official Care
Home names and addresses provided by the Care Inspectorate.

## Usage

``` r
get_slf_ch_name_lookup_path(update = latest_update(), ...)
```
...
The path to the Care Home lookup as an
