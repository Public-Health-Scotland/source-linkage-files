...
This will read and process the all Care Home extract, it will return the
...
- sc_demog_lookup:
  ...
  The Social Care Demographics lookup produced by
...
- refined_death:
  ...
  The processed lookup of deaths from IT produced with
  [`process_refined_death()`](https://public-health-scotland.github.io/source-linkage-files/reference/process_refined_death.md).

- ch_name_lookup_path:

  Path to the Care Home name Lookup Excel workbook.

- spd_path:

  (Optional) Path the Scottish Postcode Directory, default is to use
