...
it_extract_path = file.path(get_slf_dir(),
                            "IT_extracts")
it_extract_anon_path = file.path(it_extract_path,
                                 "anon-chi-IT")
...
convert_it_csv_to_parquet = function(csv_file) {
  ...
  new_file = file.path(it_extract_anon_path,
                       paste0("anon-", basename(parequet_file)))
  if (is_chi_in_file(csv_file)) {
    data = data %>%
      dplyr::rename_with( ~ paste0("chi"),
...
check_hscdiip_available_size = function(){
  ...
  csv_files_size = paste0(round(file.size(csv_files) %>% sum() / 1024^3, 1),
                          "G")
