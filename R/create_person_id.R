...
create_person_id <- function(data, type = c("SC")) {
  ...
  if (type == "SC") {
    if (!all(
      c("sending_location", "social_care_id") %in% names(data)
    )) {
      cli::cli_abort(
        message = "The variables {.var sending_location} and
    ...
    dplyr::mutate(
      data,
      person_id = paste0(.data$sending_location, "-", .data$social_care_id)
    )
  } else {
    data
  }
