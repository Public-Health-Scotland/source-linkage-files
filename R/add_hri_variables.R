...
flag_non_scottish_residents <- function(data,
  ...
  return_data <- data %>%
    ...
    dplyr::mutate(dummy_postcode = .data$postcode %in% c("BF010AA", "NF1 1AB", "NK010AA") |
...
add_hri_variables <- function(data,
  ...
  hri_lookup <- data %>%
    ...
    dplyr::mutate(scotland_cost = sum(health_net_cost),
    ...
    dplyr::mutate(hb_cost = sum(health_net_cost),
    ...
    # LCA
    dplyr::group_by(lca) %>%
    dplyr::mutate(lca_cost = sum(health_net_cost),
