#' Uplift costs for extract tests
#'
#' @param data The extract to process
#'
#' @return extract data with uplifted costs
#' @family process extracts
apply_cost_uplift_extract <- function(data) {
  data <- data %>%
    # attach a uplift scale as the last column
    lookup_uplift_extract() %>%
    dplyr::mutate(
      # Only uplift cost variables that exist at extract stage
  ...
  cli::cli_alert_info("Apply extract cost uplift function finished at {Sys.time()}")
  return(data)
}

#' Set uplift scale for extract stage
#'
#' @param data The extract to process
#'
#' @return extract data with uplifted scale
#' @family process extracts
lookup_uplift_extract <- function(data) {
