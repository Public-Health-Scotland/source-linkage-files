...
process_sc_all_alarms_telecare <- function(
  ...
  data[,
       service_end_date := fix_sc_missing_end_dates(service_end_date,
                                                    period_end_date)]
  data[,
       service_start_date := fix_sc_start_dates(service_start_date,
                                                period_start_date)]
  data[,
       service_end_date := fix_sc_end_dates(service_start_date,
                                            service_end_date,
                                            period)]
  ...
  data$person_id = paste0(data$sending_location,
                          "-",
                          data$social_care_id)
  ...
  data[, pkg_count := seq_len(.N), by = .(sending_location,
                                          social_care_id,
                                          record_keydate1,
                                          smrtype,
                                          period)]
