...
create_individual_file <- function(
  ...
  if (!check_year_valid(year, type = c("ch", "hc", "at", "sds"))) {
    individual_file <- individual_file %>%
      dplyr::mutate(
        ch_cis_episodes = as.integer(NA),
        ch_beddays = as.numeric(NA),
        ch_cost = as.numeric(NA),
        hc_episodes = as.integer(NA),
        hc_personal_episodes = as.integer(NA),
        hc_non_personal_episodes = as.integer(NA),
        hc_reablement_episodes = as.integer(NA),
        hc_total_cost = as.numeric(NA),
        hc_total_hours = as.numeric(NA),
        hc_personal_hours = as.numeric(NA),
        hc_non_personal_hours = as.numeric(NA),
        hc_reablement_hours = as.numeric(NA),
        hc_non_personal_hours_cost = as.numeric(NA),
        hc_personal_hours_cost = as.numeric(NA),
        hc_reablement_hours_cost = as.numeric(NA),
        at_alarms = as.integer(NA),
        at_telecare = as.integer(NA),
        sds_option_1 = as.integer(NA),
        sds_option_2 = as.integer(NA),
        sds_option_3 = as.integer(NA),
        sds_option_4 = as.integer(NA),
