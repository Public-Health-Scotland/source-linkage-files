...
run_year_extract_tests <- function(year) {
  ...
  ## acute
  process_tests_acute(
    read_file(get_source_extract_path(year, "acute")),
    year
  )
  ## ae
  process_tests_ae(
    read_file(get_source_extract_path(year, "ae")),
    year
  )
  ## cmh
  process_tests_cmh(
    read_file(get_source_extract_path(year, "cmh")),
    year
  )
  ## dd
  process_tests_delayed_discharges(
    read_file(get_source_extract_path(year, "dd")),
    year
  )
  ## dn
  process_tests_district_nursing(
    read_file(get_source_extract_path(year, "dn")),
    year
  )
  ## homelessness
  ...
  process_tests_maternity(
    read_file(get_source_extract_path(year, "maternity")),
    year
  )
  ## mental_health
  process_tests_mental_health(
    read_file(get_source_extract_path(year, "mh")),
    year
  )
  ## nrs_death
  ...
  process_tests_gp_ooh(
    read_file(get_source_extract_path(year, "gp_ooh")),
    year
  )
  ## outpatients
  process_tests_outpatients(
    read_file(get_source_extract_path(year, "outpatients")),
    year
  )
  ## prescribing
  process_tests_prescribing(
    read_file(get_source_extract_path(year, "pis")),
    year
  )
  ## sc_client
  ...
  process_tests_alarms_telecare(
    read_file(get_source_extract_path(year, type = "at")),
    year
  )
  ## care_home
  process_tests_care_home(
    read_file(get_source_extract_path(year, type = "ch")),
    year
  )
  ## home care
  process_tests_home_care(
    read_file(get_source_extract_path(year, type = "hc")),
    year
  )
  ## sds
  process_tests_sds(
    read_file(get_source_extract_path(year, type = "sds")),
    year
  )
}
