...
add files
   /file = !file + 'acute_for_source-20' + !FY + '.sav'
...
match files file = *
   /table = !Extracts + 'LTCs_patient_reference_file.zsav'
   /by chi.
execute.
...
Do if arth_date GT '20180331'
   Compute arth = 0.
   Compute arth_date = ''.
End If.
Do if asthma_date GT '20180331'
   Compute asthma = 0.
   Compute asthma_date = ''.
End If.
Do if atrialfib_date GT '20180331'
   Compute atrialfib = 0.
   Compute atrialfib_date = ''.
End If.
Do if cancer_date GT '20180331'
   Compute cancer = 0.
   Compute cancer_date = ''.
End If.
Do if cvd_date GT '20180331'
   Compute cvd = 0.
   Compute cvd_date = ''.
End If.
Do if liver_date GT '20180331'
   Compute liver = 0.
   Compute liver_date = ''.
End If.
Do if copd_date GT '20180331'
   Compute copd = 0.
   Compute copd_date = ''.
End If.
Do if dementia_date GT '20180331'
   Compute dementia = 0.
   Compute dementia_date = ''.
End If.
Do if diabetes_date GT '20180331'
   Compute diabetes = 0.
   Compute diabetes_date = ''.
End If.
Do if epilepsy_date GT '20180331'
   Compute epilepsy = 0.
   Compute epilepsy_date = ''.
End If.
Do if chd_date GT '20180331'
   Compute chd = 0.
   Compute chd_date = ''.
End If.
Do if hefailure_date GT '20180331'
   Compute hefailure = 0.
   Compute hefailure_date = ''.
End If.
Do if ms_date GT '20180331'
   Compute ms = 0.
   Compute ms_date = ''.
End If.
Do if parkinsons_date GT '20180331'
   Compute parkinsons = 0.
   Compute parkinsons_date = ''.
End If.
Do if refailure_date GT '20180331'
   Compute refailure = 0.
   Compute refailure_date = ''.
End If.
Do if congen_date GT '20180331'
   Compute congen = 0.
   Compute congen_date = ''.
End If.
Do if bloodbfo_date GT '20180331'
   Compute bloodbfo = 0.
   Compute bloodbfo_date = ''.
End If.
Do if endomet_date GT '20180331'
   Compute endomet = 0.
   Compute endomet_date = ''.
End If.
Do if digestive_date GT '20180331'
   Compute digestive = 0.
   Compute digestive_date = ''.
End If.

*Match on deceased flags and date.
match files file = *
   /table = !Extracts + 'Deceased_patient_reference_file.zsav'
   /by chi.
execute.
...
Do If (recid eq '02B' and any(mpat,'1','3','5','7','A')).
   Compute SMRType = 'Matern-IP'.
Else If (recid eq '02B' and any(mpat,'2','4','6')).
...
* Update for age.
if (recid eq 'PIS') age = trunc((20170930 - dob)/10000).

if sysmis(dob) or sysmis(age) age = 999.


save outfile = '/conf/sourcedev/Anita_temp/source-episode-file-'+ !FY + '.zsav'
...
get file = '/conf/sourcedev/Anita_temp/source-episode-file-'+ !FY + '.zsav'.
...
insert file='/conf/irf/11-Development team/Dev00-PLICS-files/2017-18/convert_date.sps'.
...
save outfile = '/conf/sourcedev/Anita_temp/source-episode-file-'+ !FY + '.sav'
   /compressed.
...
get file = '/conf/sourcedev/Anita_temp/source-episode-file-'+ !FY + '.sav'.
*get file = !file + 'source-episode-file-' + !FY + '.sav'.
...
save outfile = '/conf/sourcedev/Anita_temp/source-episode-file-' +!FY +'_NRS_PIS_AE_OP.sav'.
...
save outfile = '/conf/sourcedev/Anita_temp/source-episode-file-' + !FY + '.sav' /compressed.
*save outfile = !file + 'source-episode-file-' + !FY + '.sav'
   /compressed.
get file = '/conf/sourcedev/Anita_temp/source-episode-file-' + !FY + '.sav'.
*get file = !file + 'source-episode-file-' + !FY + '.sav'.
...
add files file=*
   /file = '/conf/sourcedev/Anita_temp/source-episode-file-' +!FY +'_NRS_PIS_AE_OP.sav'
   /by CHI.
execute.
...
*get file = !file + 'source-episode-file-' + !FY + '.sav'.
get file = '/conf/sourcedev/Anita_temp/source-episode-file-' + !FY + '.sav'.
save outfile = '/conf/sourcedev/Anita_temp/source-episode-file-20' + !FY + '.sav'.
erase file = '/conf/sourcedev/Anita_temp/source-episode-file-' + !FY + '.sav'.
*RUN CHECKS.
