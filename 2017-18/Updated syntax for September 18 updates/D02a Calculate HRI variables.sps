...
get file = !File + "temp-source-individual-file-3-" + !FY + ".zsav"
   /Keep year chi postcode gpprac lca hbres Health_net_cost Health_net_costincincomplete.


String PCArea (A3).
 * Workout the postcode 
If postcode NE '' PCArea = char.substr(postcode, 1, index(postcode, "0123456789", 1) - 1).
...
if ((postcode = "") OR (!SMRDummyPC)) and SysMiss(gpprac) ScotFlag = 2.
...
If Not(SysMiss(gpprac)) Eng_Prac = 0.
...
string NonScot (A7).
if Scotflag = 0 NonScot = postcode.
