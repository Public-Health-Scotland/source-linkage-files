################################################################################
# Name of file - UAT_gp_ooh_diagnosis.R
#
...
dataset_name <- "gp_ooh_diagnosis"

# Read in Test data
sdl_view <- as_tibble(dbGetQuery(
  denodo_connect,
  "select * from sdl.sdl_gp_ooh_diagnosis_source LIMIT 100"
))

# Read boxi data
boxi_view <- read_extract_ooh_diagnosis(year = "1920")
...
denodo_vars <- readxl::read_excel('/conf/sourcedev/Source_Linkage_File_Updates/uat_testing/SLF_variable_lookup.xlsx',
                                  sheet = 'gp_ooh_diagnosis')


#-------------------------------------------------------------------------------

## Create Output --------
gp_ooh_diagnosis_output <- create_uat_output(dataset_name = dataset_name,
                               boxi_data = boxi_view,
                               sdl_data = sdl_view,
...
gp_ooh_diagnosis_output %>%
  write_uat_tests(sheet_name = "gp_ooh_diagnosis")
