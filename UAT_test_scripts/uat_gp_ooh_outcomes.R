################################################################################
# Name of file - UAT_gp_ooh_outcomes.R
#
...
# Dataset name for ID
dataset_name <- "gp_ooh_outcomes"

# Read in Test data
sdl_data <- as_tibble(dbGetQuery(
  denodo_connect,
  "select * from sdl.sdl_gp_ooh_outcome_source LIMIT 100"
))

# Read boxi data
boxi_data <- read_extract_ooh_outcomes(year = "1920")
...
## Create Output --------
gp_ooh_outcomes_output <- create_uat_output(
