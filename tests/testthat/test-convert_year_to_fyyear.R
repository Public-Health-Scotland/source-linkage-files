...
test_that("year to fyyear errors properly on bad inputs", {
  ...
  expect_error(
    convert_year_to_fyyear(NA),
    "`year` must not be \"NA\""
  )
  expect_error(
    convert_year_to_fyyear(NA_character_),
    "`year` must not be \"NA\""
  )
  expect_error(
    convert_year_to_fyyear(NA_real_),
    "`year` must not be \"NA\""
  )
  expect_error(
    convert_year_to_fyyear(c("1718", NA)),
    "`year` must not be \"NA\""
  )
  expect_error(
    convert_year_to_fyyear(c("2017", NA)),
    "`year` must not be \"NA\""
  )
})
...
test_that("Will respond correctly to weird inputs", {
  expect_equal(
    ...
    expect_warning(
      "A value was not in the 21st century i.e. not \"20xx\".+?\"1917\" -> \"1718\""
    )
})
