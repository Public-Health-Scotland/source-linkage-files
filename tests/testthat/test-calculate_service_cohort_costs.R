test_that("Costs are calculated correctly", {
  ...
  # Limited daycases
  expect_equal(
    test_output$limited_daycases_cost,
    c(0, 10, 0, 0, 0, 0, 10)
  )
  # Routine daycases
  expect_equal(
    test_output$routine_daycase_cost,
    c(0, 0, 10, 0, 0, 0, 10)
  )
  # Single emergency
  expect_equal(
    test_output$single_emergency_cost,
    c(0, 0, 0, 10, 0, 0, 20)
  )
  # Multiple emergency
  expect_equal(
    test_output$multiple_emergency_cost,
    c(0, 0, 0, 0, 10, 0, 20)
  )
  # Community care
  expect_equal(
    test_output$community_care_cost,
    c(0, 0, 0, 0, 0, 10, 30)
  )
  # Residential care (not used)
  expect_equal(
    test_output$residential_care_cost,
    c(0, 0, 0, 0, 0, 0, 0)
  )
  # Test that cost_total_net has been set to zero
  expect_equal(
    test_output$cost_total_net,
    c(0, 0, 0, 0, 0, 0, 0)
  )
})
